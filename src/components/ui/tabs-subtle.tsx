  ...
  ({ children, selectedIndex, onSelect, idPrefix, activeLabel = false, className, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        <TabsPrimitive.Root value={String(selectedIndex)} onValueChange={(value) => onSelect(Number(value))} activationMode="manual">
          <TabsPrimitive.List ref={(node: HTMLDivElement | null) => {
            containerRef.current = node;
            if (typeof ref === "function") ref(node);
            else if (ref) (ref as React.MutableRefObject<HTMLDivElement | null>).current = node;
          }} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} onFocus={(e: React.FocusEvent<HTMLDivElement>) => {
            ...
            {selectedRect && (
              <motion.div
                className={cn("absolute pointer-events-none", shape.bg)}
                initial={false}
                animate={{
                ...
                style={{ backgroundColor: 'color-mix(in srgb, var(--color-foreground) 20%, transparent)' }}
              />
            )}
            <AnimatePresence>
